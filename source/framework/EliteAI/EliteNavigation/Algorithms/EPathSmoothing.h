	...
	public:
		...
		static std::vector<Elite::Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int i = 1; i < static_cast<unsigned int>(portals.size()); ++i)
			{
				...
				if (Elite::Cross(newRightLeg, rightLeg) <= 0.f)
				{
					//2. See if new line degenerates a line segment - RIGHT
					if (Elite::Cross(newRightLeg, leftLeg) > 0.f)
					{
					...
					else
					{
						...
						auto newIt = apexIndex + 1;
						i = newIt;
						leftLegIndex = newIt;
						rightLegIndex = newIt;

						vPath.push_back(apex);
						if (static_cast<unsigned int>(newIt) < static_cast<unsigned int>(portals.size()))
						{
				...
				if (Elite::Cross(newLeftLeg, leftLeg) >= 0.f)
				{
					//2. See if new line degenerates a line segment - LEFT
					if (Elite::Cross(newLeftLeg, rightLeg) < 0.f)
					{
						leftLeg = newLeftLeg;
						leftLegIndex = i;
					}
			...
			vPath.push_back((portals.end() - 1)->Line.p1);
			return vPath;
		}
	private:
		SSFA() {};
