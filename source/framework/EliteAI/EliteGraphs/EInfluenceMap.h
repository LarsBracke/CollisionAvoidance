...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		{
			ConnectionList connectionList = GetNodeConnections(pNode);
			float highestNewInfluence{ 0.f };
			for (GraphConnection* pConnection : connectionList)
			{
				InfluenceNode* pConnectedNode = GetNode(pConnection->GetTo());
				float newInfluence =
					pConnectedNode->GetInfluence() * expf(-(pConnection->GetCost()) * m_Decay);

				if (abs(newInfluence) > abs(highestNewInfluence)) highestNewInfluence = newInfluence;
			}

			m_InfluenceDoubleBuffer[pNode->GetIndex()] =
				m_Momentum * pNode->GetInfluence() + (1 - m_Momentum) * highestNewInfluence;
		}

		for (unsigned int index = 0; index < m_InfluenceDoubleBuffer.size(); ++index)
		{
			m_Nodes[index]->SetInfluence(m_InfluenceDoubleBuffer[index]);
		}
	}
